...
const FormScreen: React.FC = () => {
    ...
    const handlePlayText = async (text: string, gender: string) => {
        try {
            await speak(text, {
                language: 'en',
                // voice: voice === 'Male' ? 'com.apple.ttsbundle.Moira-compact' : 'com.apple.ttsbundle.Samantha-compact',
            });
        } catch (error) {
    ...
    //     try {
    //         const response = await axios.post('http://localhost:3000/convertToSpeech', {
    //             texts: formData, // Combine all text inputs into one string
    //         }, {
